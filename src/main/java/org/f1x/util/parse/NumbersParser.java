...
import org.f1x.message.parser.FixParserException;
import org.f1x.util.buffer.Buffer;

public class NumbersParser {
    public static int parseInt(Buffer buffer, int offset, int length) {
        if (length == 0)
        ...
        for (int i=0; i < length; i++) {
            byte ch = buffer.getByte(offset+i);
            if (ch >= '0' && ch <= '9')
    ...
    public static int parsePositiveInt (Buffer buffer, int offset, int length) {
        int result = 0;
        for (int i=0; i < length; i++) {
    ...
    public static long parseLong(Buffer buffer, int offset, int length) {
        if (length == 0)
            throw new FixParserException("Number cannot be empty");
    ...
    /** Parses double value. Adaptation of algorithm published by Jean-Marie Dautell */
     public static double parseDouble(Buffer buffer, int offset, int length) {
         if (length == 0)
             throw new FixParserException("Number cannot be empty");
        try {
            double result = 0.0;
            ...
            boolean isNegative = (buffer.getByte(offset) == '-');
            int i = (isNegative || (buffer.getByte(offset) == '+')) ? offset+1 : offset;
