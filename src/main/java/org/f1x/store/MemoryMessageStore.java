...
public class MemoryMessageStore implements MessageStore {
    ...
    protected static final int ALIGNMENT = SIZE_OF_LONG;
    protected static final int MIN_CAPACITY = 1 << 10;

    protected static final int EXTRA_SIZE = 4 * SIZE_OF_INT + SIZE_OF_LONG;
    ...
    protected int claim(int required) {
        ...
        if (continuous < required) {
            ...
            buffer.putInt(capacity - SIZE_OF_INT, entrySize);
    ...
    protected long lowerBound(int seqNumBound) {
        ...
        while (head > tail) {
            int index = mask(head - SIZE_OF_INT);
            int entrySize = buffer.getInt(index);
            ...
            if (head >= tail) {
                index = mask(head);
                int seqNum = buffer.getInt(index + SIZE_OF_INT);
